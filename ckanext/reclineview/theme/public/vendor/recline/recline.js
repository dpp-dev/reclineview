...
(function(my) {
  ...
  var _wrapInTimeout = function(ourFunction) {
    ...
    var timer = setTimeout(function() {
      dfd.reject({
        message: 'Pieprasījuma kļūda: Sistēma neatbildēja ilgāk par ' + (my.timeout / 1000) + ' sekundēm'
      });
    }, my.timeout);
    ...
    <div class="recline-flot"> \
      <div class="panel graph" style="display: block;"> \
        <div class="js-temp-notice alert alert-warning alert-block"> \
          <h3 class="alert-heading">Sveicināti!</h3> \
          <p>Garfiks vēl nav izveidots, jo mēs nezinām, kurus datu laukus tajā attelot.</p> \
          <p>Lūdzu, norādiet tos, izmantojot izvēlni pa labi un grafiks automātiski tiks parādīts.</p> \
        </div> \
      </div> \
    </div> \
',
    ...
    <form class="form-stacked"> \
      <div class="clearfix"> \
        <div class="form-group"> \
          <label>Grafika tips</label> \
          <div class="input editor-type"> \
            <select class="form-control"> \
              <option value="lines-and-points">Līnijas un punkti</option> \
              <option value="lines">Līnijas</option> \
              <option value="points">Punkti</option> \
              <option value="bars">Stabiņi</option> \
              <option value="columns">Kolonas</option> \
            </select> \
          </div> \
        </div> \
        <div class="form-group"> \
          <label>Grupas kolona (Ass 1)</label> \
      ...
      <div class="editor-buttons"> \
        <button class="btn btn-default editor-add">Pievienot sēriju</button> \
      </div> \
      <div class="editor-buttons editor-submit" comment="hidden temporarily" style="display: none;"> \
        <button class="editor-save">Saglabāt</button> \
...
',
  templateSeriesEditor: ' \
    <div class="editor-series js-series-{{seriesIndex}}"> \
      <div class="form-group"> \
        <label>Sērija <span>{{seriesName}} (Ass 2)</span> \
          [<a href="#remove" class="action-remove-series">Noņemt</a>] \
        </label> \
        <div class="input"> \
          <select class="form-control"> \
          {{#fields}} \
...
my.GridRow = Backbone.View.extend({
  ...
  template: ' \
      ...
      <td data-field="{{field}}" style="width: {{width}}px; max-width: {{width}}px; min-width: {{width}}px;"> \
        <div class="data-table-cell-content"> \
          <a href="javascript:{}" class="data-table-cell-edit" title="Rediģēt šo šūnu">&nbsp;</a> \
    ...
    <div class="menu-container data-table-cell-editor"> \
      ...
      <div id="data-table-cell-editor-actions"> \
        <div class="data-table-cell-editor-action"> \
          <button class="okButton btn primary">Atjaunot</button> \
          <button class="cancelButton btn danger">Atcelt</button> \
        </div> \
      </div> \
    </div> \
  ',
  ...
  onEditorOK: function(e) {
    ...
    this.trigger('recline:flash', {message: "Atjauno rindu...", loader: true});
    this.model.save().then(function(response) {
        this.trigger('recline:flash', {message: "Rinda atjaunota veiksmīgi", category: 'success'});
      })
      .fail(function() {
        this.trigger('recline:flash', {
          message: 'Kļūda saglabājot rindu',
          category: 'error',
          persist: true
        });
      });
...
',
  ...
  latitudeFieldNames: ['lat','LOCATION_LATITUDE'],
  longitudeFieldNames: ['lon','LOCATION_LONGITUDE'],
  ...
  _geomReady: function() {
    return Boolean(this.state.get('geomField') || (this.state.get('latField') && this.state.get('lonField')) || (this.state.get('eField') && this.state.get('nField')));
  },
    ...
    _.every(docs, function(doc){
      ...
      if (typeof feature === 'undefined' || feature === null){
        ...
        try {
          ...
          wrongSoFar += 1;
          var msg = 'Nepareiza ģeometrijas vērtība';
        ...
        wrongSoFar += 1;
        if (wrongSoFar <= 10) {
          self.trigger('recline:flash', {message: 'Nepareiza ģeometrijas vērtība', category:'error'});
        }
      }
      return true;
    });
  ...
  //CONVERT COORD START
   _lks_RadToDeg:function (rad)    {
	  var pi = 3.14159265358979;
        return (rad / pi * 180.0)
    },
  
    _lks_DegToRad:function (deg)
    {
	 var pi = 3.14159265358979;
        return (deg / 180.0 * pi)
    },
	
	_lks_FootpointLatitude:function (y)
    {
    ...
    var sm_EccSquared = 6.69437999013e-03;
	 ...
	 _lks_MapXYToLatLon:function (x, y, lambda0, philambda)
    {
    ...
    var sm_EccSquared = 6.69437999013e-03;
        ...
        phif =this._lks_FootpointLatitude (y);
    ...
    _lks_UTMXYToLatLon:function(x, y, zone, southhemi, latlon)    {
        var cmeridian;
	...
	y -= -6000000.0;
        ...
        cmeridian = this._lks_UTMCentralMeridian (zone);
        this._lks_MapXYToLatLon (x, y, cmeridian, latlon);
        	
        return;
    },
  
    _lks_UTMCentralMeridian:function(zone)
    {
        var cmeridian;

        cmeridian = this._lks_DegToRad (24.0);
...
//CONVERT COORD END
  ...
  _getGeometryFromRecord: function(doc){
    if (this.state.get('geomField')){
      ...
      if (typeof(value) === 'string') {
        ...
        var lat = this._parseCoordinateString(parts[2]);
		...
		this._lks_UTMXYToLatLon (x, y, zone, southhemi, latlon);

		var n=this._lks_RadToDeg(latlon[1]);
		var e=this._lks_RadToDeg (latlon[0]);
	...
	if (this.state.get('eField') && this.state.get('nField')){
      // We'll create a GeoJSON like point object from the two lat/lon fields
			
      var lon = doc.get(this.state.get('nField'));
      var lat = doc.get(this.state.get('eField'));
	  var x, y, zone, southhemi;
	  var latlon = new Array(2);
     ...
     var e=this._lks_RadToDeg (latlon[0]);
      ...
      lon = this._parseCoordinateString(n);
      lat = this._parseCoordinateString(e);
  ...
  _setupMap: function(){
    ...
    var mapUrl = this.options.mapTilesURL || 'http://{s}.tile.openstreetmap.org/{z}/{x}/{y}.png';
    ...
    this.map.setView([56.94797027510018, 24.11224365234375], 7);
...
my.MapMenu = Backbone.View.extend({
    ...
    <form class="form-stacked"> \
      <div class="clearfix"> \
        ...
        <div class="editor-field-type-geom" style="display:none"> \
          <label>Ģeometrijas lauks (GeoJSON)</label> \
      ...
      <div class="editor-buttons"> \
        <button class="btn btn-default editor-update-map">Ielādēt kartē</button> \
      </div> \
      <div class="editor-options" > \
        <label class="checkbox"> \
          <input type="checkbox" id="editor-auto-zoom" value="autozoom" checked="checked" /> \
          Automātiskā tuvināšana</label> \
        <label class="checkbox"> \
          <input type="checkbox" id="editor-cluster" value="cluster"/> \
          Ķekarošanas marķieri</label> \
      </div> \
  ...
  render: function() {
    ...
    if (this._geomReady() && this.model.fields.length){
	  ...
	  else if (this.state.get('lonField' && 'latField')){
        this._selectOption('editor-lon-field',this.state.get('lonField'));
        this._selectOption('editor-lat-field',this.state.get('latField'));
        this.$el.find('#editor-field-type-latlon').attr('checked','checked').change();
      } else {
        this._selectOption('editor-e-field',this.state.get('eField'));
        this._selectOption('editor-n-field',this.state.get('nField'));
        this.$el.find('#editor-field-type-lks92en').attr('checked','checked').change();
      }
    }
    if (this.state.get('autoZoom')) {
  ...
  onEditorSubmit: function(e){
	...
	if (this.$el.find('#editor-field-type-latlon').attr('checked')){
      this.state.set({
        geomField: null,
	...
	if (this.$el.find('#editor-field-type-lks92en').attr('checked')){

      this.state.set({
        geomField: null,
		lonField: null,
        latField: null,
        eField: this.$el.find('.editor-e-field > select > option:selected').val(),
        nField: this.$el.find('.editor-n-field > select > option:selected').val()
      });
  ...
  onFieldTypeChange: function(e){
    if (e.target.value == 'geom'){
		...
		this.$el.find('.editor-field-type-lks92en').hide();
	...
	if (e.target.value == 'lks92en'){
		this.$el.find('.editor-field-type-lks92en').show();
        this.$el.find('.editor-field-type-geom').hide();
        this.$el.find('.editor-field-type-latlon').hide();
    }
  },
  ...
  <div class="recline-data-explorer"> \
    ...
    <div class="header clearfix"> \
      ...
      <div class="recline-results-info"> \
        <span class="doc-count">{{recordCount}}</span> ieraksti\
      </div> \
...
my.SlickGrid = Backbone.View.extend({
  initialize: function(modelEtc) {
    ...
    this.templates = {
      "deleterow" : '<button href="#" class="recline-row-delete btn btn-default" title="Dzēst rindu">X</button>'
    };
...
my.GridControl= Backbone.View.extend({
  ...
  template: '<h1><button href="#" class="recline-row-add btn btn-default">Pievienot rindu</button></h1>',
...
my.FilterEditor = Backbone.View.extend({
  ...
  template: ' \
    <div class="filters"> \
      <h3>Filtri</h3> \
      <a href="#" class="js-add-filter">Pievienot filtru</a> \
      <form class="form-stacked js-add" style="display: none;"> \
        ...
        <div class="form-group"> \
          <label>Filtra tips</label> \
          <select class="filterType form-control"> \
            <option value="term">Vārtība</option> \
            <option value="range">Robeža</option> \
            <option value="geo_distance">Geo robeža</option> \
          </select> \
        </div> \
      ...
      <div class="filter-{{type}} filter"> \
        <fieldset> \
          <legend> \
            ...
            <a class="js-remove-filter" href="#" title="Noņemt šo filtru" data-filter-id="{{id}}">&times;</a> \
          </legend> \
  ...
  template: ' \
    <div class="pagination"> \
      <ul class="pagination"> \
        ...
        <li class="page-range"><a><label for="from">No</label><input id="from" name="from" type="text" value="{{from}}" /> &ndash; <label for="to">Līdz</label><input id="to" name="to" type="text" value="{{to}}" /> </a></li> \
    ...
    <form action="" method="GET" class="form-inline" role="form"> \
      <div class="form-group"> \
        <div class="input-group text-query"> \
          ...
          <input class="form-control search-query" type="text" id="q" name="q" value="{{q}}" placeholder="Meklēt datus ..."> \
        </div> \
      </div> \
...
my.ValueFilter = Backbone.View.extend({
  ...
  template: ' \
    <div class="filters"> \
      <h3>Filtri</h3> \
      <button class="btn js-add-filter add-filter">Pievienot filtru</button> \
      <form class="form-stacked js-add" style="display: none;"> \
        <fieldset> \
          <label>Lauks</label> \
          <select class="fields form-control"> \
